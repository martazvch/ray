...
pub fn subtract(self: *Self, range: Range, allocator: Allocator) ArrayList(Range) {
    // Uncovered fragments of range
    var out: ArrayList(Range) = .empty;
...
pub fn add(self: *Self, allocator: Allocator, range: Range) void {
    var low = range.low;
    var high = range.high;
...
pub fn addAll(self: *Self, allocator: Allocator, ranges: []const Range) void {
    for (ranges) |range| {
        self.add(allocator, range);
    }
}

pub fn checkRange(self: *Self, allocator: Allocator, range: Range) Error!void {
...
test "single scalar" {
    ...
    try std.testing.expectEqualDeep(covered[0], Range{ .low = 0, .high = 0 });
}

test "continous scalar" {
    const ranges: []const Range = &.{
    ...
    try std.testing.expect(covered.len == 1);
    try std.testing.expectEqualDeep(covered[0], Range{ .low = 0, .high = 1 });
}

test "discontinous scalar" {
    const ranges: []const Range = &.{
    ...
    try std.testing.expectEqualDeep(covered[0], Range{ .low = 0, .high = 0 });
    try std.testing.expectEqualDeep(covered[1], Range{ .low = 3, .high = 3 });
}
...
test "single range" {
    ...
    try std.testing.expect(covered.len == 1);
    try std.testing.expectEqualDeep(covered[0], Range{ .low = 2, .high = 4 });
}
...
test "continous range" {
    ...
    try std.testing.expect(covered.len == 1);
    try std.testing.expectEqualDeep(covered[0], Range{ .low = 0, .high = 8 });
}
...
test "discontinous ranges" {
    ...
    try std.testing.expect(covered.len == 2);
    try std.testing.expectEqualDeep(covered[0], Range{ .low = 3, .high = 6 });
    try std.testing.expectEqualDeep(covered[1], Range{ .low = 8, .high = 12 });
}

test "errors" {
    var ranges: []const Range = &.{
        .{ .low = 3, .high = 6 }, .{ .low = 5, .high = 8 },
    };
    _ = runTestCheck(ranges) catch |err| {
        try std.testing.expect(err == Error.partial);
    };
...
fn runTestCheck(ranges: []const Range) Error![]const Range {
    const allocator = std.testing.allocator;
    var covered: Self = .empty;
